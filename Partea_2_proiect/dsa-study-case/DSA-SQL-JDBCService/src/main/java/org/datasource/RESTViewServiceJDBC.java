...
import org.datasource.jdbc.views.transactions.TransactionView;
import org.datasource.jdbc.views.transactions.TransactionViewBuilder;
...
public class RESTViewServiceJDBC {
    ...
    @RequestMapping(value = "/TransactionsView", method = RequestMethod.GET,
            produces = {MediaType.APPLICATION_JSON_VALUE, MediaType.APPLICATION_XML_VALUE})
    @ResponseBody
    public List<TransactionView> get_TransactionView() {
        List<TransactionView> viewList = transactionViewBuilder.build().getViewList();
        return viewList;
    }
	// Set-up
    ...
    @Autowired private TransactionViewBuilder transactionViewBuilder;
	//
}