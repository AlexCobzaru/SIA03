...
import org.datasource.mongodb.views.clientrisk.ClientRiskView;
import org.datasource.mongodb.views.clientrisk.ClientRiskViewBuilder;
...
public class RESTViewServiceMongoDB {
    ...
    @RequestMapping(value = "/ClientRiskView", method = RequestMethod.GET,
            produces = {MediaType.APPLICATION_JSON_VALUE, MediaType.APPLICATION_XML_VALUE})
    @ResponseBody
    public List<ClientRiskView> get_ClientRiskView() {
        List<ClientRiskView> viewList = this.clientRiskViewBuilder.build().getViewList();
        return viewList;
    }

	// Set-up 
	@Autowired private DepartamentViewBuilder viewBuilder;
    @Autowired private ClientRiskViewBuilder clientRiskViewBuilder;
}